...
export default function BatchPreviewTable({ previewData, selectedRows, onToggleRow, onToggleAll }) {
    if (!previewData || !previewData.detalles) return null;

    const allSelected = selectedRows && previewData.detalles.filter(f => f.is_valid).length > 0 && 
                        selectedRows.size === previewData.detalles.filter(f => f.is_valid).length;
            ...
            <div className="p-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                ...
                <div className="flex gap-4 text-sm">
                    <span className="flex items-center gap-1 text-blue-600 font-bold">
                        <FaCheckCircle /> {selectedRows?.size || 0} Seleccionados
                    </span>
            ...
            <div className="overflow-x-auto max-h-[500px]">
                <table className="w-full text-left border-collapse text-sm">
                    <thead className="bg-gray-100 text-gray-600 sticky top-0 z-10 shadow-sm">
                        <tr>
                            <th className="py-3 px-4 border-b w-10">
                                <input 
                                    type="checkbox" 
                            ...
                            <th className="py-3 px-4 border-b">Propietario</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {previewData.detalles.map((fila, idx) => (
                            <tr 
                                key={idx} 
                                className={`hover:bg-gray-50 transition-colors ${!fila.is_valid ? 'bg-red-50 opacity-60' : ''} ${selectedRows?.has(idx) ? 'bg-indigo-50' : ''}`}
                            >
                                ...
                                <td className="py-3 px-4 text-xs text-gray-500 truncate max-w-[150px]">{fila.unidad_propietario || '-'}</td>
