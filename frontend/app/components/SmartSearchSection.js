...
export default function SmartSearchSection() {
    ...
    useEffect(() => {
        ...
        if (query.startsWith(':')) {
            const cmdQuery = lowerQuery.slice(1);
            const matches = commandDictionary.filter(cmd =>
                cmd.aliases.some(a => a.startsWith(cmdQuery)) ||
                cmd.triggers.some(t => t.includes(cmdQuery))
            ).map(cmd => ({ ...cmd, isCommand: true }));
        ...
        // Search in Commands (by name/trigger)
        const cmdMatches = commandDictionary.filter(cmd =>
            cmd.name.toLowerCase().includes(lowerQuery) ||
            cmd.triggers.some(t => t.includes(lowerQuery))
        );
        matches = [...matches, ...cmdMatches];

        // Search in Navigation Menu
        menuStructure.forEach(section => {
            if (section.links) {
                const linkMatches = section.links.filter(link =>
                    link.name.toLowerCase().includes(lowerQuery) ||
                    (link.description && link.description.toLowerCase().includes(lowerQuery))
                );
                matches = [...matches, ...linkMatches];
            }
            if (section.subgroups) {
                section.subgroups.forEach(sub => {
                    const linkMatches = sub.links.filter(link =>
                        link.name.toLowerCase().includes(lowerQuery)
                    );
                    matches = [...matches, ...linkMatches];
                });
