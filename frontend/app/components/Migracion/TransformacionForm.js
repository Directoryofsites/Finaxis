...
export default function TransformacionForm({ tiposDocumento, cuentas = [] }) {
  ...
  const [transformRules, setTransformRules] = useState({
    ...
    dateRangeFrom: '',
    dateRangeTo: '',
    replaceAccountFrom: '', // CÓDIGO de cuenta origen
    replaceAccountTo: '',   // CÓDIGO de cuenta destino
  });
  ...
  const handleTransformFileLoad = async (e) => {
    ...
    try {
      ...
      // --- Soporte Híbrido (Transacciones / Documentos / Backups Firmados) ---
      let rawDocs = jsonData.transacciones || jsonData.documentos;

      // Si el backup viene del "Generar Backup" (FIRMADO), los datos están en jsonData.data
      if (!rawDocs && jsonData.data) {
        rawDocs = jsonData.data.transacciones || jsonData.data.documentos;
      }

      if (!rawDocs) {
        throw new Error("El archivo JSON no contiene la sección 'transacciones' ni 'documentos'. Revise si es un formato compatible.");
      }
  ...
  const handleApplyTransformations = () => {
    ...
    try {
      ...
      const newBackupData = {
        ...transformData,
        // Si el original era firmado, actualizamos el nodo 'data'
        ...(transformData.data ? {
          data: {
            ...transformData.data,
          ...
          signature: "MODIFIED_INVALID_SIGNATURE" // Invalidamos la firma explícitamente
        } : {
          // Si era formato plano
          transacciones: documentosTransformados,
          documentos: documentosTransformados
        })
      };
    ...
    <>
      ...
      <div className="bg-white p-8 rounded-xl shadow-lg border border-gray-100 mt-8 animate-fadeIn">
        ...
        <div className={`transition-opacity duration-300 ${!transformData ? 'opacity-50 pointer-events-none grayscale' : 'opacity-100'}`}>
          ...
          {/* Panel 3: Reemplazo de Cuentas (NUEVO) */}
          <div className="mt-8 border border-purple-200 p-6 rounded-xl bg-purple-50 shadow-sm animate-fadeIn">
            <h4 className="text-sm font-bold text-purple-700 uppercase mb-4 border-b border-purple-100 pb-2 flex items-center gap-2">
              <FaExchangeAlt /> Reemplazo Masivo de Cuentas Contables
            </h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-end">
              <div>
                <label className={labelClass}>Cuenta a Reemplazar (Origen)</label>
                <select
                  name="replaceAccountFrom"
                  value={transformRules.replaceAccountFrom}
                  onChange={handleRuleChange}
                  className={selectClass}
                >
                  <option value="">-- No reemplazar ninguna --</option>
                  {cuentas.map(c => (
              ...
              <div>
                <label className={labelClass}>Nueva Cuenta (Destino)</label>
                <select
                  name="replaceAccountTo"
                  value={transformRules.replaceAccountTo}
                  onChange={handleRuleChange}
                  className={selectClass}
                >
            ...
            <p className="mt-3 text-xs text-purple-600 italic">
              * Esta regla buscará todos los movimientos contables de los documentos seleccionados y cambiará el código de cuenta automáticamente.
            </p>
          </div>
        </div>
