...
// If '../config/rightSidebarConfig' does not exist, you might need to adjust this import.
...
export default function RightSidebar({ isOpen, isPinned, onToggle, onPin, onClose }) {
    ...
    // FIXED: Definición de sendCalcToNote que faltaba
    const sendCalcToNote = () => {
        const val = display === 'Error' ? '0' : display;
        setCurrentText(prev => {
            const separator = prev ? '\n' : '';
            return `${prev}${separator}Calculadora: ${formatNumber(val)}`;
        });
        setActiveTab('notes');
        toast.success("Enviado a notas");
    };
    ...
    const handleAiSubmit = async (e, forcedQuery = null) => {
        ...
        try {
            ...
            if (response.status === 401) {
                // Debug fallback logic if needed
            }
                    ...
                    {activeTab === 'ai' && (
                        <div className="flex flex-col h-full">
                            ...
                            <form onSubmit={(e) => handleAiSubmit(e)} className="bg-white p-2 rounded-2xl shadow-sm border border-gray-200 mt-4 relative">
                                ...
                                {/* HISTORY POPUP */}
                                {showHistory && commandHistory.length > 0 && !aiQuery && (
                                    <div className="absolute bottom-full left-0 right-0 mb-2 bg-white border border-gray-100 rounded-xl shadow-lg z-50 animate-fadeIn">
                                        <div className="p-2 text-[10px] text-gray-400 font-bold uppercase border-b border-gray-50 flex items-center gap-2"><FaClock /> Historial</div>
                                        {commandHistory.map((cmd, idx) => (
                                            <div key={idx} onClick={() => handleHistoryClick(cmd)} className="px-3 py-2 hover:bg-indigo-50 cursor-pointer text-xs text-gray-600 truncate border-b border-gray-50 last:border-0 flex items-center gap-2">
                                                <FaHistory className="text-gray-300" /> {cmd}
                                            </div>
                                        ))}
                                    </div>
                                )}
