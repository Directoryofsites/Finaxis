...
# El contenido que DEBE estar al final (La función PDF)
pdf_function_code = """
...
try:
    ...
    # 3. Limpiar basura del final (Si el append anterior falló)
    # Buscamos el final de la función anterior 'get_cartera_ph_pendientes_detallada'
    # Esta función termina en "return lista_final"
    
    marker = "return lista_final"
    last_idx = text.rfind(marker)
    
    if last_idx != -1:
        # Cortamos justo despues del return lista_final
        # Asumiendo indentación estandar + saltos de linea
        cutoff = last_idx + len(marker)
        clean_text = text[:cutoff]
        ...
        print("File repaired and code appended correctly.")
    else:
        print("CRITICAL: Marker not found. Saving cleaned version only.")
