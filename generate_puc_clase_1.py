...
OUTPUT_FILE = r"c:\ContaPY2\Manual\maestros\PUC_Clase_1_Activos.json"
...
def parse_activos_txt():
    ...
    for i in range(len(matches)):
        ...
        # FILTRO ESTRICTO: Solo cuentas que empiezan por '1' (Activos)
    ...
    print(f"Se encontraron {len(cuentas_raw)} cuentas potenciales (Filtradas solo Clase 1).")
    return cuentas_raw

def generate_puc_clase_1():
    print("Construyendo PUC Maestro - CLASE 1 (ACTIVOS)...")
    
    cuentas_raw = parse_activos_txt()
    if not cuentas_raw:
    ...
    for item in temp_list:
        ...
        if nivel > 1:
            ...
            # Si no se encuentra el padre directo, intentar buscar hacia arriba (opcional, pero estricto por ahora)
            # En PUC estricto, el padre debe existir.

        # Determinar si permite movimiento (Nivel 4 o superior, o si no tiene hijos - lógica simplificada: nivel >= 4)
        # Ajuste: En PUC comercial, generalmente nivel 4 (6 digitos) o 5 (8 digitos) son auxiliares.
        # Asumiremos que nivel >= 4 (6 dígitos o más) permite movimiento.
    ...
    backup_data = {
        "metadata": {
            ...
            "version_sistema": "7.0",
            "empresa_id_origen": 0,
            "descripcion": "PLANTILLA MAESTRA PUC - CLASE 1 ACTIVOS"
        },
        "empresa": {}, 
        "configuracion": {}, 
        "maestros": {
