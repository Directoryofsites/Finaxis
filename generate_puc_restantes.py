...
# CONFIGURACIÓN
# =============================================================================
INPUT_FILE = r"c:\ContaPY2\Manual\maestros\resto 7 8 9.txt"
OUTPUT_DIR = r"c:\ContaPY2\Manual\maestros"
...
def parse_txt():
    ...
    print(f"Se encontraron {len(cuentas_raw)} cuentas potenciales en total.")
    return cuentas_raw

def generate_puc_for_class(target_class, class_name, all_accounts):
    print(f"\nProcesando CLASE {target_class} ({class_name})...")
    
    # Filtrar cuentas para esta clase
    cuentas_clase = [
        (c, n) for c, n in all_accounts 
        if c.startswith(str(target_class))
    ]
    
    if not cuentas_clase:
        print(f"No se encontraron cuentas para la Clase {target_class}.")
        return
    ...
    temp_list = []
    for codigo, nombre in cuentas_clase:
    ...
    # Estructura Final
    filename = f"PUC_Clase_{target_class}_{class_name.replace(' ', '_')}.json"
    ...
    backup_data = {
        "metadata": {
            ...
            "version_sistema": "7.0",
            "empresa_id_origen": 0,
            "descripcion": f"PLANTILLA MAESTRA PUC - CLASE {target_class} {class_name}"
        },
        "empresa": {}, 
        "configuracion": {}, 
        "maestros": {
    ...
    print(f"Total cuentas: {len(plan_cuentas)}")

def generate_all():
    all_accounts = parse_txt()
    if not all_accounts:
        return

    for clase_num, clase_name in CLASSES_CONFIG.items():
        generate_puc_for_class(clase_num, clase_name, all_accounts)
